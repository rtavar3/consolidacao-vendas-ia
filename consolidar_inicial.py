...
def limpar_dados(df: pd.DataFrame) -> pd.DataFrame:
    """Padroniza nomes de colunas, remove duplicatas e trata nulos."""
    df.columns = [c.strip().lower().replace(" ", "_") for c in df.columns]
    df = df.drop_duplicates()
    # TODO: adaptar tratamento de nulos e datas ao seu caso real
...
def gerar_resumo(df: pd.DataFrame) -> pd.DataFrame:
    """Gera agregações relevantes. TODO: adaptar colunas ao seu caso real."""
    # exemplo genérico — troque pelas colunas do seu domínio
